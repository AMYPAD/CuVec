...
PyMODINIT_FUNC PyInit_cuvec(void) {
  ...
  if (PyType_Ready(&Vector_c.type_obj) < 0) return NULL;
  Py_INCREF(&Vector_c.type_obj);
  PyModule_AddObject(m, Vector_c.name.c_str(), (PyObject *)&Vector_c.type_obj);

  if (PyType_Ready(&Vector_b.type_obj) < 0) return NULL;
  ...
  if (PyType_Ready(&Vector_h.type_obj) < 0) return NULL;
  ...
  if (PyType_Ready(&Vector_i.type_obj) < 0) return NULL;
  ...
  if (PyType_Ready(&Vector_q.type_obj) < 0) return NULL;
  ...
  if (PyType_Ready(&Vector_f.type_obj) < 0) return NULL;
  Py_INCREF(&Vector_f.type_obj);
  PyModule_AddObject(m, Vector_f.name.c_str(), (PyObject *)&Vector_f.type_obj);

  if (PyType_Ready(&Vector_d.type_obj) < 0) return NULL;
